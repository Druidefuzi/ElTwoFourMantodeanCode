...
namespace AddSpawn
{
    public class OwnPawnSpawnComp : ThingComp
    {
        ...
        {
            FactionDef factionDef = this.Props.myFaction;
            if (factionDef == null)
            {
                factionDef = FactionDef.Named("PlayerColony");
            }
            Faction faction = FactionUtility.DefaultFactionFrom(factionDef);
            Verse.PawnGenerationRequest request = new Verse.PawnGenerationRequest(this.Props.Pawnkind, faction, PawnGenerationContext.NonPlayer, fixedBiologicalAge : Props.StartingAge, fixedChronologicalAge : Props.StartingAge);
